...
open class ConsoleStyle @Inject constructor(private val messenger: Messenger) : Messenger {
    fun success(recipient: Any, message: String, parameters: Set<Any> = emptySet()) =
        this.send(recipient, "&2✓&8 $message", parameters)

    fun error(recipient: Any, message: String, parameters: Set<Any> = emptySet()) =
        this.send(recipient, "&4✗&c $message", parameters)

    fun warning(recipient: Any, message: String, parameters: Set<Any> = emptySet()) =
